...
| Host | User | Status |
|---|---|---|
| `lewis` | `jorge` | xr-driver + breezy-gnome + breezy-session + breezy-recenter + **monado-rayneo + breezy-hyprland + glasses-edid (EDID override + USB ACL fix)** deployed. GNOME-Breezy soak abandoned (productivity-tier paywall). **Phase 3 SOLVED 2026-05-06**: actual cause of "SURFACE_LOST" was a launcher race — the socket-wait loop saw a stale `monado_comp_ipc` from a previous run and broke immediately, so wayvr launched before monado-service had finished init and got `Connection refused`. monado was then killed by EXIT trap before any frame landed, leaving a half-released DRM state that *looked* like a SURFACE_LOST in the v1 trace. Fix: `rm -f` the stale socket in the launcher before starting monado-service (commit `f74154b`). v3 diagnostic post-fix shows steady-state present cycle, zero SURFACE_LOST, ~60fps presents at 120Hz target. The earlier "Mesa anv missing VK_KHR_display device funcs" framing was a separate misdiagnosis — see `memory/xr-mesa-anv-display-gap.md`. |
