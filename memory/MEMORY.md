...
- [lewis EC refuses CAM registration entirely (deeper failure)](xr-lewis-ec-refuses-altmode.md) — recurring wedge: `GET_CAM_SUPPORTED=0` despite partner advertising SVID 0xff01. **Don't trust UCSI alone** — it can misreport while i915 DP path engages anyway; cross-check `hyprctl monitors -j` / `/sys/class/drm/card1-DP-*/status`. 5-command UCSI recipe + recovery notes inside. Recovered 2026-05-08 with cause unknown.
