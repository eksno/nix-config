...
- [lewis EC refuses CAM registration entirely (deeper failure)](xr-lewis-ec-refuses-altmode.md) — recurring wedge: `GET_CAM_SUPPORTED=0` despite partner advertising SVID 0xff01. **Don't trust UCSI alone** — it can misreport while i915 DP path engages anyway; cross-check `hyprctl monitors -j` / `/sys/class/drm/card1-DP-*/status`. 5-command UCSI recipe inside. **BIOS UX3405MA 301→311 flashed 2026-05-21 — wedge UNCHANGED on first plug; Slimbook precedent provisionally falsified for this bug on this model.**
