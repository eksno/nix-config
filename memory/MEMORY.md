...
- [lewis EC refuses CAM registration entirely (deeper failure)](xr-lewis-ec-refuses-altmode.md) — **RECOVERED 2026-05-22**: BIOS .311 + drop `acpi.ec_no_wakeup=1` + drop `drm.edid_firmware=...` EDID firmware override + reboot to gen 62. Glasses now a real DP-2 monitor (1920x1080@120, real EDID "Technical Concepts Ltd SmartGlasses"). UCSI debugfs STILL shows wedge fingerprint while DP works — don't trust UCSI alone. Causal isolation between the 3 changes pending. The previous "lockout in EC firmware flash, BIOS is only recovery" claim was at best incomplete.
- [Untested: post-suspend `-70` errors → fresh replug](xr-ec-altmode-suspend-replug-untested.md) — single specific timing not yet falsified for the EC wedge above. Cheap (~30 sec) test before BIOS flash; details + falsification recipe inside.
