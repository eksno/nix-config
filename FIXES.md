...
## 2026-07-15 — corne-silent-both-transports-was-charge-only-cable-plus-wiped-host-bond

**Symptom:** Corne "again — not connecting or seemingly advertising, without usb.. well I have a usb but it didn't connect it." No BLE, and cabling it did nothing either.
**Affected:** verse/eksno. Corne `C8:5B:C1:B5:9B:F3`, host adapter `2C:33:58:44:27:4D`. Runtime recovery only (no repo file changed). Keymap combos live in `~/repos/eksno/corne-36-dvorak/config/corne.keymap` (moved from the old `zmk-dvorak-36` path).
**Root cause:** Two independent benign causes that together mimicked dead hardware. (1) The USB cable is **charge-only** — the board runs off its 5V but there are no data lines, so the host logged **zero** USB events on plug-in (no enumeration, no `1d50:615e`, no bootloader drive). (2) The host-side bond had been **removed entirely** at 14:48 (bond dir had no `C8:5B` entry — unlike past *keyless* desyncs), leaving **profile 1 one-sided**: the keyboard held verse's LTK but verse held nothing, so it couldn't re-encrypt and never completed a connection, and it wasn't advertising *as pairable* on its identity address either (private-address directed advertising the host could no longer resolve).
**Investigation:**
1. `fastfetch` → verse/eksno. Grepped FIXES for `corne` — all prior entries are BLE bond desyncs where the device stayed *known*; this time `bluetoothctl info C8:5B…` → "not available" and the on-disk bond dir had no Corne entry. Different shape.
2. Host confirmed healthy: adapter Powered/Pairable, scan works. 8s scan found no `C8:5B` and no USB input device / no `ttyACM`.
3. Live `dmesg -w` across a 30s re-plug window: **zero** new USB events; USB tree showed only webcam + BT controller. Repeated after a second plug-in — still nothing. This *looked* like a dead central half.
4. Power-cycled both halves; still no BLE advertising, so leaned (wrongly) toward flat battery. `CONFIG_ZMK_SLEEP=n` (deep sleep disabled to dodge the nice!nano drain bug) made "flat by afternoon" plausible and reinforced the wrong theory.
5. **Turning point:** user sent photos. The **nice!view display was rendering** — profile 1 selected, layer "home", a healthy battery graph. So the board was alive the whole time → USB silence = charge-only cable; BLE silence = bond problem, not power.
