{ config, pkgs, ... }:
{
  ...
  boot.kernelParams = [
    ...
    # "acpi.ec_no_wakeup=1"       # TEST 2026-05-21: dropped to let EC wake on s2idle resume
    #                                so the suspend→`-70`-UCSI-errors→replug hypothesis can fire.
    #                                See memory/xr-ec-altmode-suspend-replug-untested.md.
    #                                Restore after test (it's the documented power-saving default).
